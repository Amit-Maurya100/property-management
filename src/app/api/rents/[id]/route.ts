import { handleApiError, jsonOk } from "@/lib/api/response";
import { getAuthenticatedPropertyContext } from "@/lib/properties/api";
import { updateRentSchema } from "@/lib/properties/schemas";
import { deleteRent, getRent, updateRent } from "@/lib/properties/rents";
import { withPermission } from "@/lib/permissions";

type Params = { params: Promise<{ id: string }> };

export const GET = withPermission(async (_request, context) => {
  try {
    const ctx = await getAuthenticatedPropertyContext();
    const { id } = await (context as Params).params;
    return jsonOk(await getRent(ctx, id));
  } catch (error) {
    return handleApiError(error);
  }
}, "rent", "read");

export const PATCH = withPermission(async (request, context) => {
  try {
    const ctx = await getAuthenticatedPropertyContext();
    const { id } = await (context as Params).params;
    const body = updateRentSchema.parse(await request.json());
    return jsonOk(await updateRent(ctx, id, body));
  } catch (error) {
    return handleApiError(error);
  }
}, "rent", "update");

export const DELETE = withPermission(async (_request, context) => {
  try {
    const ctx = await getAuthenticatedPropertyContext();
    const { id } = await (context as Params).params;
    await deleteRent(ctx, id);
    return jsonOk({ success: true });
  } catch (error) {
    return handleApiError(error);
  }
}, "rent", "delete");
